...
// Definimos los tipos permitidos para el tipo de negocio
type BusinessType = "store" | "restaurant" | "pharmacy";

export function Register() {
  const navigate = useNavigate(); // Hook de React Router para redireccionar
  const { register } = useAuth(); // Hook personalizado para registrar usuarios
  const [loading, setLoading] = useState(false); // Indica si está en proceso de registro
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register(formData); // Registrar usuario con datos del formulario
              ...
              <div>
                ...
                <div className="mt-1">
                  ...
                  >
                    ...
                    {/* Este valor no está en el tipo BusinessType, podrías añadirlo */}
                  </select>
                </div>
              </div>
